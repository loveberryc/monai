...
class ProjMetric(CumulativeIterationMetric):
    """
...
def compute_meanproj(
    ...
    projd = np.empty((batch_size, n_class))
    y_pred1 = np.empty((batch_size, n_class,y_pred.shape[3],y_pred.shape[4]))
    y_pred2 = np.empty((batch_size, n_class,y_pred.shape[2],y_pred.shape[3]))
    y1 = np.empty((batch_size, n_class,y_pred.shape[3],y_pred.shape[4]))
    y2 = np.empty((batch_size, n_class,y.shape[2],y.shape[3]))

    if y.shape != y_pred.shape:
        raise ValueError(f"y_pred and y should have same shapes, got {y_pred.shape} and {y.shape}.")

    for b, c in np.ndindex(batch_size, n_class):
        y_pred1[b, c] = np.max(y_pred[b, c],axis=0)
        y_pred2[b, c] = np.max(y_pred[b, c], axis=2)
        y1[b, c] = np.max(y[b, c],axis=0)
        y2[b, c] = np.max(y[b, c],axis=2)
        s1 = []
        s2 = []
        for i in range(len(y_pred1[b, c])):
            t=[]
            tp=[]
            for j in range(len(y_pred1[b, c][i])):
                if y_pred1[b, c][i][j] >= 0.5:
                    tp.append(j)
                if y1[b, c][i][j] >= 0.5:
                    t.append(j)
            if tp and t:
                ...
                label=max(t)-min(t)
            try:
                s1.append(abs(label-pred))
            except:
                s1.append(np.mean(s1))
        for i in range(np.size(y_pred2[b, c],1)):
            t=[]
            tp=[]
            for j in range(np.size(y_pred2[b, c],0)):
                if y_pred2[b, c][j][i] >= 0.5:
                    tp.append(j)
                if y2[b, c][j][i] >= 0.5:
                    t.append(j)
            if tp and t:
                ...
                label=max(t)-min(t)
            try:
                s2.append(abs(label-pred))
            except:
                s2.append(np.mean(s2))
        projd[b, c] = (np.mean(s1) + np.mean(s2))/2
    return torch.from_numpy(projd)