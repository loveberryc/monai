...
class HD3to2Loss(nn.Module):
    """Binary Hausdorff loss based on morphological erosion 3D to 2D"""

    def __init__(self, alpha=2.0, erosions=10, **kwargs):
        super(HD3to2Loss, self).__init__()
        self.alpha = alpha
        self.erosions = erosions
        self.prepare_kernels()
    ...
    def forward(
        ...
        predy = pred.max(3)[0]
        predz = pred.max(4)[0]
        print(predx.shape, predy.shape, predz.shape)
        targetx = target.max(2)[0]
        targety = target.max(3)[0]
        targetz = target.max(4)[0]

        erotedx = torch.from_numpy(
            self.perform_erosion(predx.cpu().detach().numpy(), targetx.cpu().detach().numpy(), debug)
        ).float()
        erotedy = torch.from_numpy(
            self.perform_erosion(predy.cpu().detach().numpy(), targety.cpu().detach().numpy(), debug)
        ).float()
        erotedz = torch.from_numpy(
            self.perform_erosion(predz.cpu().detach().numpy(), targetz.cpu().detach().numpy(), debug)
        ).float()

        loss = erotedx.mean() + erotedy.mean() + erotedz.mean()
        return loss


class HD3to2DTLoss(nn.Module):
    """Binary Hausdorff loss based on distance transform"""

    def __init__(self, alpha=2.0, **kwargs):
        super(HD3to2DTLoss, self).__init__()
        self.alpha = alpha
...
#         print(predx.shape, predy.shape, predz.shape)
        ...
        # pred = torch.sigmoid(pred)
        target = one_hot(target, num_classes=8)
        predx = pred.logsumexp(2)[0].unsqueeze(0)
        predy = pred.logsumexp(3)[0].unsqueeze(0)
        predz = pred.logsumexp(4)[0].unsqueeze(0)
        print(predx.shape, predy.shape, predz.shape)
        targetx = target.logsumexp(2)[0].unsqueeze(0)
        targety = target.logsumexp(3)[0].unsqueeze(0)
        targetz = target.logsumexp(4)[0].unsqueeze(0)
        print(pred.shape)
        print(target.shape)

        predx_dt = torch.from_numpy(self.distance_field(predx.detach().cpu().numpy())).float()
        targetx_dt = torch.from_numpy(self.distance_field(targetx.detach().cpu().numpy())).float()
        predy_dt = torch.from_numpy(self.distance_field(predy.detach().cpu().numpy())).float()
        targety_dt = torch.from_numpy(self.distance_field(targety.detach().cpu().numpy())).float()
        predz_dt = torch.from_numpy(self.distance_field(predz.detach().cpu().numpy())).float()
        targetz_dt = torch.from_numpy(self.distance_field(targetz.detach().cpu().numpy())).float()

        predx_error = (predx - targetx) ** 2
        distancex = predx_dt ** self.alpha + targetx_dt ** self.alpha
        dtx_field = predx_error * distancex.cuda()

        predy_error = (predy - targety) ** 2
        distancey = predy_dt ** self.alpha + targety_dt ** self.alpha
        dty_field = predy_error * distancey.cuda()

        predz_error = (predz - targetz) ** 2
        distancez = predz_dt ** self.alpha + targetz_dt ** self.alpha
        dtz_field = predz_error * distancez.cuda()

        loss = dtx_field.mean() + dty_field.mean() + dtz_field.mean()

        return loss


def distance_transform_edt1(input):

    # calculate the feature transform

    edge = edt(input)
    edge = np.atleast_1d(np.where(edge==1, 1, 0).astype(np.int8))
    input = np.atleast_1d(np.where(input, 1, 0).astype(np.int8))
    dt = np.zeros(input.shape, dtype=np.int32)
    for l in range(len(input)):
        op=[]
        for i in range(len(input[l])):
            st=[]
            for j in range(len(input[l][i])):
                if edge[l][i][j]==1:
                    st.append(j)
                    op.append(i)
            for r in range(len(input[l][i])):
                tt=[]
                if input[l][i][r]==1 and st:
                    for d in st:
                        tt.append(abs(d-r)+1)
                else:
                    tt=[0]
        ...
        if op:
            ...
            ma=max(op)
            for i in range(len(input[l])):
                if not dt[l][i].any() and input[l][i].all():
                    if i<mi:
                        dt[l][i]=dt[l][i]+dt[l][mi]+mi-i
                    elif i>ma:
                        dt[l][i]=dt[l][i]+dt[l][ma]+i-ma
...
def distance_transform_edt2(input):
...
class HD3to2DTNewLoss(nn.Module):
    """Binary Hausdorff loss based on distance transform"""

    def __init__(self, alpha=2.0, **kwargs):
        super(HD3to2DTNewLoss, self).__init__()
        self.alpha = alpha

    @torch.no_grad()
    def distance_field1(self, img: np.ndarray) -> np.ndarray:
        field = np.zeros_like(img)
    ...
    @torch.no_grad()
    def distance_field2(self, img: np.ndarray) -> np.ndarray:
        field = np.zeros_like(img)
        ...
        for batch in range(len(img)):
            ...
            if fg_mask.any():
                ...
                fg_dist = distance_transform_edt1(fg_mask)
                bg_dist = distance_transform_edt1(bg_mask)
    ...
    @torch.no_grad()
    def distance_field3(self, img: np.ndarray) -> np.ndarray:
        field = np.zeros_like(img)
        ...
        for batch in range(len(img)):
            ...
            if fg_mask.any():
                ...
                fg_dist = distance_transform_edt2(fg_mask)
                bg_dist = distance_transform_edt2(bg_mask)
    ...
    def forward(
        ...
        predx_dt = torch.from_numpy(self.distance_field2(predx.detach().cpu().numpy())).float()
        targetx_dt = torch.from_numpy(self.distance_field2(targetx.detach().cpu().numpy())).float()
        predy_dt = torch.from_numpy(self.distance_field1(predy.detach().cpu().numpy())).float()
        targety_dt = torch.from_numpy(self.distance_field1(targety.detach().cpu().numpy())).float()
        predz_dt = torch.from_numpy(self.distance_field3(predz.detach().cpu().numpy())).float()
        targetz_dt = torch.from_numpy(self.distance_field3(targetz.detach().cpu().numpy())).float()
...
class HD3to2DTNewLSELoss(nn.Module):
    """Binary Hausdorff loss based on distance transform"""

    def __init__(self, alpha=2.0, **kwargs):
        super(HD3to2DTNewLSELoss, self).__init__()
        self.alpha = alpha
...
class DiceCELoss(_Loss):
    ...
    def __init__(
        ...
        self.DT = HausdorffDTLoss()
        self.ER = HausdorffERLoss()
        self.HDemb = HD3to2Loss()
        self.HD3to2DTLoss = HD3to2DTLoss()
        self.HD3to2DTNewLoss = HD3to2DTNewLoss()
        self.HD3to2DTNewLSELoss = HD3to2DTNewLSELoss()
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.HD == 0:
            ...
            DT_loss = self.DT(input, target)
            alpha = ce_loss / DT_loss
            alpha=alpha.detach()
            print(alpha)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * DT_loss
        elif self.HD == 2:
            ER_loss = self.ER(input, target)
            alpha = ce_loss / ER_loss
            alpha=alpha.detach()
            print(alpha)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * ER_loss
        elif self.HD == 3:
            HDemb_loss = self.HDemb(input, target)
            alpha = ce_loss / HDemb_loss
            alpha=alpha.detach()
            print(alpha)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * HDemb_loss
        elif self.HD == 4:
            HDDT_loss = self.HD3to2DTLoss(input, target)
            alpha = ce_loss / HDDT_loss
            alpha=alpha.detach()
            print(alpha)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * HDDT_loss
        elif self.HD == 5:#不用纯max了
            HDDTnew_loss = self.HD3to2DTNewLoss(input, target)
            alpha = ce_loss / HDDTnew_loss
            alpha=alpha.detach()
            print(alpha)
#             alpha=10**(-7)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * HDDTnew_loss
        elif self.HD == 6:
            HDDTnewLSE_loss = self.HD3to2DTNewLSELoss(input, target)
            alpha = ce_loss / HDDTnewLSE_loss
            alpha=alpha.detach()
            print(alpha)
#             alpha=10**(-7)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * HDDTnewLSE_loss
        elif self.HD == 7:
            HDDTnewLSE_loss = self.HD3to2DTNewLSELoss(input, target)
            print(self.lambda_dice)
#             alpha=10**(-7)
            total_loss: torch.Tensor = self.lambda_dice * (dice_loss + ce_loss) + (1 - self.lambda_dice) * HDDTnewLSE_loss
        elif self.HD == 8:
            HDDT_loss = self.HD3to2DTLoss(input, target)
            alpha = (ce_loss+dice_loss) / HDDT_loss
            alpha=alpha.detach()
            print(alpha)
            total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_ce * ce_loss + alpha * HDDT_loss
        elif self.HD == 9:#从dice80开始训练
            HDDT_loss = self.HD3to2DTLoss(input, target)
            self.lambda_dice = 0.001
            print(self.lambda_dice)
            total_loss: torch.Tensor = self.lambda_dice * (dice_loss + ce_loss) + (1 - self.lambda_dice) * HDDT_loss
        elif self.HD == 10:#从dice80开始训练
            DT_loss = self.DT(input, target)
            self.lambda_dice = 0.001
            print(self.lambda_dice)
            total_loss: torch.Tensor = self.lambda_dice * (dice_loss + ce_loss) + (1 - self.lambda_dice) * DT_loss
        elif self.HD == 11:
            HDDT_loss = self.HD3to2DTLoss(input, target)
            print(self.lambda_dice)
#             alpha=10**(-7)
            ...
            HDDT_loss = self.HD3to2DTLoss(input, target)
#             alpha=10**(-7)
            total_loss: torch.Tensor = awl(HDDT_loss, dice_loss,ce_loss)
        elif self.HD == 13:
            rw = RWingLoss()
            HDDT_loss = self.HD3to2DTLoss(input, target)
